...
class InitialCheck(wx.Frame):
 ...
 #       DemoBox = self.Demogbox(panel)
        ...
        Dose = self.LabelEntry(panel,'Dose discrepancy?: YES(how much in %), NO, UNKNOWN',widgetname='Dose',binding=(wx.EVT_KEY_UP,self.OnDemogValidate))
...
#        w = self.widgets
        with open(fpath,'w') as f:
          ...
          #  f.write('On')
            ...
            f.write(SL(',',self.widgets['LocDis']))
            f.write(' found that ')
            f.write(SL('on',self.widgets['DateHap']))
            f.write(SL(' at',self.widgets['LocHap']))
            f.write(', the following event occured for the patient')
            f.write(SL('',self.widgets['Name']))
            f.write(SL(' U# (',self.widgets['UNum']))
            f.write('):')
            f.write(SL('',self.widgets['NarSum']))
            
            
            f.write('\n')
            f.write(SL('This occurred',self.widgets['When']))
            f.write('. The following staff were informed/involved: ')
     ...
     #       digit = lambda x: int(filter(self.widgets['Dose'].isdigit, x) or 0)
            answer = self.widgets['Dose'].GetValue()
            why = self.widgets['why'].GetValue()
          #  digit = answer.encode('ascii','ignore')

            if 'y' in answer.lower():
                digit = re.findall('\d+',answer)
                if not digit:
                    f.write("There was a dose discrepancy of XXX")
                else:
                  # digit =itertools.chain(*digit)
                    digit[0] = float(digit[0])
                    if len(digit)==2:
                        digit[1] = float(digit[1])
                        f.write("There was a dose discrepancy of %d.%d" %(digit[0], digit[1]) )
                    else:
                        f.write("There was a dose discrepancy of %d" %digit[0] )
                            
                #f.write(digit)
                f.write('% to the treatment delivery. ')
            else:
                f.write(SL('There was',self.widgets['Dose']))
                f.write(' dose discrepancy to the treatment delivery. ')
            if not why:
                return True
            else:    
                f.write(SL('This incident was probably caused by',self.widgets['why']))
                f.write('\n')
